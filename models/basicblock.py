...
class ResBlock(nn.Module):
    def __init__(self, in_channels=64, out_channels=64, temb_dim=None, kernel_size=3, stride=1, padding=1, bias=True, mode='CRC', negative_slope=0.2):
        super(ResBlock, self).__init__()

        assert in_channels == out_channels, 'Only support in_channels==out_channels.'

        if temb_dim is not None:
            self.Dense_0 = sequential(nn.SiLU(),nn.Linear(temb_dim, out_channels))
        if mode[0] in ['R', 'L']:
    ...
    def forward(self, x, temb=None):
        if temb is not None:
            x0 = x + self.Dense_0(temb)[:, :, None, None]
        return x + self.res(x0)
...
# Attention BLock
# --------------------------------------------
# attention block with GroupNorm
# --------------------------------------------
'''
...
# --------------------------------------------
# Get time embedding for continuous and 
# discrete time steps
# --------------------------------------------
'''
...
# --------------------------------------------
# get_timestep_embedding is ported over from the DDPM codebase:
